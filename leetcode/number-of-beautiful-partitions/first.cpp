...
public:
    ...
    int dfs(int i, bool at_start, int k, string& s, int ans = 0) {
        if (i == s.size())
            return k == 0;
        if (i > s.size() || k == 0 || !prime.count(s[i]) && at_start)
            return 0;
        if (dp[i][at_start][k] != -1)
            return dp[i][at_start][k];
        if (prime.count(s[i])) {
            if (at_start)
                ans = dfs(i + minLength - 1, false, k, s);
            else
                ans = dfs(i + 1, false, k, s);
        } else
            ans = (dfs(i + 1, true, k - 1, s) + dfs(i + 1, false, k, s)) % mod;
