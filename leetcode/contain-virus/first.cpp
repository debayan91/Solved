...
public:
    void dfs(vector<vector<int>>& grid, int i, int j, vector<vector<bool>>& vis,
        ...
        if (i >= grid.size() || i < 0 || j < 0 || j >= grid[0].size() ||
            vis[i][j] || grid[i][j] == -1) {
            // if the current src is out of grid bounds or if the cell is
            // already visited  or current cell is already walled that is -1
            return;
        }
        if (grid[i][j] == 0) {
            // if cell is uncontaminated then add to the uncontaminatd cells
            // (uc) and increment wall cnt
            cl.wcnt++;
            cl.uc.insert({i, j});
            return;
        }
        // if current cell is contaminated add to contaminated and mark vis =
        // true
        cl.c.insert({i, j});
        vis[i][j] = true;
    ...
    int containVirus(vector<vector<int>>& grid) {
        ...
        while (1) {
            ...
            // comparator for pq to sort it according to size of uncontaminated
            // cells in cluster
            ...
            // if size of priority queue is 0 the termination condition is
            // reached ie all contaminated cells are contained.
            if (pq.size() == 0)
                break;
            // get the biggest cluster is found make em all -1 and add the walls
            // reqd to contain them
            cluster k = pq.top();
            pq.pop();
            for (auto x : k.c) {
            ...
            // for the rest of the clusters make their uncontaminated cells as
            // contaminated
            while (!pq.empty()) {
                cluster k1 = pq.top();
                pq.pop();
